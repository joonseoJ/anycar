...
if DEBUGGING:
    from mujoco_collect.visualize_dataset import animate_dataset
...
latent_dim = model_config.MODEL_DIM
...
# architecture = 'vanilla_transformer'
# architecture = 'mlp'
architecture = 'HOT_transformer'
...
if architecture == 'vanilla_transformer':
    ...
    model = TmpMLP(latent_dim, state_dim, num_heads, num_layers, dropout, name=architecture)
elif architecture == 'HOT_transformer':
    model = JaxHOTDynamicsPredictor(latent_dim, latent_dim*4, state_dim, num_heads, num_layers, dropout, name=architecture)
...
def huber_loss(x, delta=1.0):
    """
    Standard Huber Loss implementation in JAX.
    0.5 * x^2                  if |x| <= delta
...
def loss_fn(
    state, var_collect, history, static_features, action, y, current_state, rngs, 
    deterministic=False,
    w_pos=1.0, w_rot=1.0, w_vel=0.1, w_ang_vel=0.1,
    w_step=1.0, w_accum=1.0):
    """
    ...
    target_delta = jax.lax.stop_gradient(y)
    action = jax.lax.stop_gradient(action)

    # Forward pass via TrainState
    pred_delta = state.apply_fn(var_collect, history, static_features, action, rngs=rngs, deterministic=deterministic)
    
    # --- Weight vector (State Dim = 12) ---
    # Pos(3), Rot(3), LinVel(3), AngVel(3)
    weights = jnp.concatenate([
        jnp.full((3,), w_pos),
        jnp.full((3,), w_rot),
        jnp.full((3,), w_vel),
        jnp.full((3,), w_ang_vel)
    ]) 
    # weights shape: (12,) -> Broadcasting will handle (B, T, E, 12) automatically

    # -----------------------------------------------------------
    # 1. Step-wise Loss (순간 변화량 오차)
    # -----------------------------------------------------------
    step_diff = pred_delta - target_delta
    step_loss_elementwise = huber_loss(step_diff)
    
    # 가중치 적용 및 평균 계산
    weighted_step_loss = step_loss_elementwise * weights
    loss_step = jnp.mean(weighted_step_loss)

    # -----------------------------------------------------------
    # 2. Cumulative Loss (누적 경로 오차) - 핵심
    # axis=1 (Time dimension)에 대해 누적합
    # -----------------------------------------------------------
    pred_cumsum = integrate_state(current_state, pred_delta) # (B, T_future, E, 13)
    target_cumsum = integrate_state(current_state, target_delta) # (B, T_future, E, 13)

    q_pred_cumsum = pred_cumsum[..., 3:7]
    q_target_cumsum = target_cumsum[..., 3:7]
    
    # q_next = q_prev * exp(so3)  =>  exp(so3) = q_prev^-1 * q_next
    q_rel = q_multiply_jax(q_inverse_jax(q_target_cumsum), q_pred_cumsum)
    delta_so3 = q_to_so3_jax(q_rel)

    accum_diff = jnp.concatenate([
        pred_cumsum[..., 0:3] - target_cumsum[..., 0:3],  # Pos diff
        delta_so3,                                      # Rot diff in so3
        pred_cumsum[..., 7:13] - target_cumsum[..., 7:13] # LinVel, AngVel diff
    ], axis=-1)
    accum_loss_elementwise = huber_loss(accum_diff)
    
    # 가중치 적용 및 평균 계산
    weighted_accum_loss = accum_loss_elementwise * weights
    loss_accum = jnp.mean(weighted_accum_loss)
...
def val_episode(var_collect, episode_num, rngs):
    ...
    for history, static_features, action, y, current_state in dataloader:
        ...
        static_features = jnp.array(static_features.numpy()) #(B, E, S)
        action = jnp.array(action.numpy()) #(B, T_future, E, A)
        y = jnp.array(y.numpy()) #(B, T_future, E, X)
        current_state = jnp.array(current_state.numpy()) #(B, E, X)
    
        # Predict (B, T_future, E, X)
        delta_predicted_states = apply_batch(var_collect, history, static_features, action, y, rngs=rngs)
        next_step_pred = integrate_state(current_state, delta_predicted_states[:, 0:1, :, :]) #(B, 1, E, X)
        predictions_list.append(np.array(next_step_pred[:, 0, :, :])) #(B, E, X)
    
    remain_trajectories = integrate_state(predictions_list[-1][0:1,:,:], delta_predicted_states[-1:, 1:, :, :]) #(B, T_future-1, E, X)
    predictions_list.append(remain_trajectories[0]) #(T_future-1, E, X)
    pred_trajectory = np.concatenate(predictions_list, axis=0)
...
def visualize_episode(epoch_num: int, episode_num, val_dataset, rngs):
    ...
    axs[0, 1].plot(predicted_states[:, 0, 7], label='Predicted vx', linestyle='dotted')
    axs[0, 1].legend()

    # 2. Vy
    axs[1, 0].plot(episode[:, 0, 8], label='Ground Truth vy')
    axs[1, 0].plot(predicted_states[:, 0, 8], label='Predicted vy', linestyle='dotted')
    axs[1, 0].legend()

    # 3. Angular Velocity (Yaw Rate)
    axs[1, 1].plot(episode[:, 0, 12], label='Ground Truth v_yaw')
    axs[1, 1].plot(predicted_states[:, 0, 12], label='Predicted v_yaw', linestyle='dotted')
    axs[1, 1].legend()

    fig.tight_layout()
    fig.savefig(f'episode_{epoch_num}_{episode_num}.png')
    plt.close(fig)
    
    # Log image to WandB
    wandb.log({f"episode_{epoch_num}_{episode_num}": wandb.Image(f'episode_{epoch_num}_{episode_num}.png')})
...
for epoch in tqdm.tqdm(range(num_epochs), desc="Epoch"):
    ...
    # --- Batch Loop ---
    for i, (history, static_features, action, y, current_state) in enumerate(t):
        if DEBUGGING:
            for b in range(history.shape[0]):
                tmp_states = np.concatenate(
                    [history[b, :, :, :state_dim], y[b,:,:,:]],
                    axis=0
                )
                tmp_actions = np.concatenate(
                    [history[b, :, :, state_dim:], action[b,:,:,:]],
                    axis=0
                )
                animate_dataset(tmp_states, tmp_actions, interval=10)
        ...
        def this_loss_fn(var_collect, history, static_features, action, y, current_state):
    ...
    ckpt = {'model': global_state, 'description': 'Inference difference of future states (B,T,E,X)'}
    save_args = orbax_utils.save_args_from_target(ckpt)
    
    # Save using step (epoch)
    checkpoint_manager.save(epoch+1, ckpt, save_kwargs={'save_args': save_args}, force=True)
