...
from car_foundation.utils import quaternion_to_euler, generate_subsequences, generate_subsequences_hf, align_yaw, q_inverse, q_multiply, q_to_so3
...
class MujocoDataset(Dataset):
    def __init__(self, path, history_length, action_length, 
        ...
        self.sequence_length = history_length + action_length + 1
    ...
    def __getitem__(self, idx):
        ...
        # 5. Augmentation (Attack / Noise)
        if self.attack:
            # Random timestep index
            noise_loc = torch.randint(0, self.sequence_length, (1,)).item()
            data_window[noise_loc, :, MujocoDataConfig.STATE_VX] += (torch.rand(data_window.shape[1]) * 60 - 30)

        if self.add_noise:
            # Add noise to Pos
            data_window[:, :, MujocoDataConfig.STATE_POS] += torch.rand_like(data_window[:, :, MujocoDataConfig.STATE_POS]) * 0.01 - 0.005
            # Twist
            data_window[:, :, MujocoDataConfig.STATE_VX] += torch.rand_like(data_window[:, :, MujocoDataConfig.STATE_VX]) * 1 - 0.5 # vx
            data_window[:, :, MujocoDataConfig.STATE_VY] += torch.rand_like(data_window[:, :, MujocoDataConfig.STATE_VY]) * 0.1 - 0.05 # vy
            data_window[:, :, MujocoDataConfig.STATE_VZ] += torch.rand_like(data_window[:, :, MujocoDataConfig.STATE_VZ]) * 0.1 - 0.05 # vz
            data_window[:, :, MujocoDataConfig.STATE_ANG_VEL] += torch.rand_like(data_window[:, :, MujocoDataConfig.STATE_ANG_VEL]) * 0.01 - 0.005 # wx, wy, wz
            # Action history
            data_window[:, :, MujocoDataConfig.HISTORY_ACTION_START:MujocoDataConfig.HISTORY_ACTION_END] += torch.rand_like(data_window[:, :, MujocoDataConfig.HISTORY_ACTION_START:MujocoDataConfig.HISTORY_ACTION_END]) * 0.05 - 0.025

        current_state = data_window[self.history_length, :, :self.state_dim]  # t 시점의 상태

        states_prev = data_window[:-1, :, :13]      # t-T_history, ...,t+T_future-1
        states_curr = data_window[1:, :, :13]       # t-T_history+1, ...,t+T_future
        actions_curr = data_window[1:, :, 13:]      # Action은 그대로 사용

        # Position Delta (3 dim)
        delta_pos = states_curr[..., 0:3] - states_prev[..., 0:3]

        # Orientation Delta (so3, 3 dim)
        q_prev = states_prev[..., 3:7]
        q_curr = states_curr[..., 3:7]
        
        # q_next = q_prev * exp(so3)  =>  exp(so3) = q_prev^-1 * q_next
        q_rel = q_multiply(q_inverse(q_prev), q_curr)
        delta_so3 = q_to_so3(q_rel)

        # Linear Velocity Delta (3 dim)
        delta_lin_vel = states_curr[..., 7:10] - states_prev[..., 7:10]

        # Angular Velocity Delta (3 dim)
        delta_ang_vel = states_curr[..., 10:13] - states_prev[..., 10:13]

        # Delta state: (Seq, E, 12)
        delta_state_and_action = torch.cat([delta_pos, delta_so3, delta_lin_vel, delta_ang_vel, actions_curr], dim=-1)

        # 5. Data Separation
        # history: (T_history, E, X+A)
        history = delta_state_and_action[:self.history_length]
        # history = data_window[:self.history_length]

        # action (Future): (B, T_future, E, A)
        action_seq = actions_curr[self.history_length-1 : self.sequence_length-2]
        # action_seq = data_window[self.history_length-1 : self.sequence_length-1, :, self.state_dim:]

        # y (Target): (B, T_future, E, X)
        y = delta_state_and_action[self.history_length : self.sequence_length, :, :self.state_dim-1]
        # y = full_data[self.history_length : self.sequence_length, :, :self.state_dim]
